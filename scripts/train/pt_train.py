...
def main(
    ...
    train_dataset.transform = TrainableLMDBTransform(train_max_num_samples, raw_dataset.max_seq_len)
    logger.info(f"{train_dataset=!r}")

    valid_dataset = raw_dataset.select(
        split_name="valid", prompt_only=prompt_only, max_num_items=None
    )
    valid_dataset.transform = TrainableLMDBTransform(valid_max_num_samples, raw_dataset.max_seq_len)
    ...
    model = DiscreteDistributionNet(backbone, max_seq_len=raw_dataset.max_seq_len)
